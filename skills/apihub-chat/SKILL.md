---
name: apihub-chat
description: Chat / code generation via apihub using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through apihub.
---
...
- `POST $APIHUB_URL/v1/chat/completions` — OpenAI format
- `POST $APIHUB_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $APIHUB_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$APIHUB_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $APIHUB_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.APIHUB_URL}/v1`, apiKey: process.env.APIHUB_KEY });
...
```bash
curl -X POST $APIHUB_URL/v1/messages \
  -H "Authorization: Bearer $APIHUB_KEY" \
