---
name: apihub-web-fetch
description: Fetch URL → markdown / text / HTML via apihub /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---
...
```bash
curl $APIHUB_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$APIHUB_URL/v1/models/info?id=firecrawl/fetch"
```
...
### Jina Reader
```bash
curl -X POST $APIHUB_URL/v1/web/fetch \
  -H "Authorization: Bearer $APIHUB_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://apihub.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.APIHUB_URL}/v1/web/fetch`, {
  method: "POST",
